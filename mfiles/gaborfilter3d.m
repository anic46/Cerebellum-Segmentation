...
tic
clear all;
I=uint8(readanalyze('datasets for 3d/603H20595-20040722.T1.byte_acpc.img'));
...
%gabout=zeros(size(img,1),size(img,2),size(img,3),12);
img=im2double(img);
...
for countphi=1:noofanglesxy
    for countsi=1:noofanglesz
    counter=counter+1;
    counter
    phi=pi/noofanglesxy*(countphi);
...
%    (theta*180)/pi ;
    ...
    for x = -gsize:gsize
       for y = -gsize:gsize
          for z=-gsize:gsize
                ...
                G(gsize+x+1,gsize+y+1,gsize+z+1) = exp(-.5*((xPrime/Sx)^2+(yPrime/Sy)^2+(zPrime/Sz)^2));
          end
        end
    end
    ...
    %Regabout=Regabout;
    %Imgabout=Imgabout;
    gabout(:,:,:,counter)=sqrt(Regabout.^2);
    %Imgabout=Imgabout-mean(mean(mean(Imgabout)))*ones(size(Imgabout));
%      if (var(var(Regabout(:,:,offset)))<10^-4)
%          Regabout(:,:,offset)=zeros(size(Regabout(:,:,offset)));
%      end
    
%      gabout(:,:,:,counter)= Regabout;%sqrt(sum(Regabout.^2,3));
end
end
end

for v=1:noofanglesxy*nooffrequencies*noofanglesz
gnet(:,:,:,v)=mean(mean(mean(gabout(:,:,:,v))))*ones(size(gabout(:,:,:,v)));
end
gabout1=gabout-gnet;


gabout2=tanh(0.25*gabout);
gabout2=abs(gabout2);
