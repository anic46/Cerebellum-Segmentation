...
%      end
       gaboutreal(:,i,:) =imfilter((Regabout),fspecial('average',[3 3]));
    gaboutimag(:,i,:) =imfilter(Imgabout,fspecial('average',[3 3]));
    end
    ...
    %max(max(max(gaboutreal)))*10
      writeanalyze(255*gaboutreal/max(max(max(gaboutreal))),strcat('dataset2_multi/featurerealyz',num2str(counter),'.img'),dim,dtype);
%      %greal=imfilter(uint8(255*gaboutimag(:,:,i)/max(max(gaboutimag(:,:,i)))+128),fspecial('gaussian',[3 3],1));
      writeanalyze(128*gaboutimag/max(max(max(gaboutimag)))+128,strcat('dataset2_multi/featureimagyz',num2str(counter),'.img'),dim,dtype);
