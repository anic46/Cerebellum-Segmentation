...
tic
clear all;
I=uint8(readanalyze('dataset6.hdr'));
    ...
    for x = -gsize:gsize
       for y = -gsize:gsize
          for z=-3:3
                ...
                G(gsize+x+1,gsize+y+1,6+z+1) = exp(-.5*((xPrime/Sx)^2+(yPrime/Sy)^2+(zPrime/Sz)^2))*exp(2i*pi*(x*f*cos(phi)*cos(si)+y*f*cos(phi)*sin(si)+z*f*sin(phi)));
          end
        end
    end
