%clc;
clear all;
%I1=readanalyze('dataset2.hdr');
%img=adapthisteq(uint8(I1(:,:,40)));
...
temp1=g;
gabout1=zeros(size(img,1),size(img,2),12);
gabout=zeros(size(img,1),size(img,2),12);
img=im2double(img);
    ...
    for offset=1:maxoffset
        ...
        for u = 1:256
            for v = 1:256
                ...
                vPrime= v*cos(theta)-u*sin(theta);
                %yPrime = y * cos(theta) + x * sin(theta);
%                G(gsize+x+1,gsize+y+1,offset) = (exp(-.5*((xPrime/Sx)^2+(yPrime/Sy)^2))*exp(2*pi*f*xPrime+pi*(offset-1)/(maxoffset+1)));
                G(u,v) = exp(-2*pi^2*((uPrime-f*cos(theta))^2*Sx^2+(vPrime-f*sin(theta))^2*Sy^2));
    ...
    convresult=G.*ftimg;
    gabout(:,:,counter)=ifft2(convresult);
    %Imgabout(:,:,offset) = conv2((I),double(real(G1)),'same');
%     Regabout(:,:,offset)= conv2((I),double(real(G(:,:,offset))),'same');
%     Imgabout(:,:,offset)=conv2(I,double(imag(G(:,:,offset))),'same');
