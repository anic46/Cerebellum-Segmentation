...
for countt=0:noofangles-1
    counter=counter+1;
    for i=1:size(I1,3)
    img=(imrotate(uint8(squeeze(I1(:,:,i))),0));
    %img=im2double(img);
    I=img;
...
%      end
    gaboutreal(:,:,i) =imfilter(Regabout,fspecial('gaussian',[3 3],1));
    gaboutimag(:,:,i) =imfilter(Imgabout,fspecial('gaussian',[3 3],1));
    end
    ...
    %max(max(max(gaboutreal)))*10
     writeanalyze(gaboutreal*255/max(max(max(gaboutreal))),strcat('dataset2_multi/featurerealxy',num2str(counter),'.img'),dim,dtype);
     %greal=imfilter(uint8(255*gaboutimag(:,:,i)/max(max(gaboutimag(:,:,i)))+128),fspecial('gaussian',[3 3],1));
     gimag=128*gaboutimag/max(max(max(gaboutimag)))+128;
     writeanalyze(gimag,strcat('dataset2_multi/featureimagxy',num2str(counter),'.img'),dim,dtype);
