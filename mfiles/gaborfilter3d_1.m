...
clear all;
tic
I=(uint8(readanalyze('dataset5.img')));
...
Sy=sqrt(log(2))/(sqrt(2)*pi*f*tan(pi/(2*noofanglesz)))
Sz=Sx/2;
...
for countphi=1:size(noofanglesxy,2)
    ...
    counter=counter+1;
    counter
    phi=pi/6*(noofanglesxy(countphi));
    ...
    disp('calculating Regabout');
     ...
     gabout(:,:,:,counter)= Regabout
     gabout1(:,:,:,counter) = Imgabout;
end
end
end
%save gabout_dataset1.mat gabout
% for i=1:11
