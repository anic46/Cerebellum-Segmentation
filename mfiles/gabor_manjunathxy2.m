function 2DOrthogonalGabor(inputimage,windowsize,fmax,fmin,noofangles,noofscales,gaborfilterradius,applygaussiansmoothing)
[I1,dim,dtype]=readanalyze(inputimage);
system(['mkdir ',inputimage,'_multi']);
...
for countt=0:noofangles-1
    ...
    y=-gsize:gsize; 
    figure,surf(x,y,G);
    writeanalyze(128*gaboutreal/max(max(max(gaboutreal)))+128,strcat(inputimage,'_multi/featurerealxy',num2str(counter),'.img'),dim,dtype);
    writeanalyze(128*gaboutimag/max(max(max(gaboutimag)))+128,strcat(inputimage,'_multi/featureimagxy',num2str(counter),'.img'),dim,dtype);
...
%     gaboutimag(i,:,:) =imfilter(Imgabout,fspecial('gaussian',[3 3]));
    ...
    writeanalyze(128*gaboutreal/max(max(max(gaboutreal)))+128,strcat(inputimage,'_multi/featurerealxz',num2str(counter),'.img'),dim,dtype);
    writeanalyze(128*gaboutimag/max(max(max(gaboutimag)))+128,strcat(inputimage,'_multi/featureimagxz',num2str(counter),'.img'),dim,dtype);
