...
int itkScalarToFractalImageFilterTest( int argc, char *argv[] )
{
  ...
  if( ImageDimension == 2 )
    {  
    typename CommandProgressUpdate2D::Pointer observer= CommandProgressUpdate2D::New();
    fractal->AddObserver( itk::ProgressEvent(), observer );
    }
  else
    {
    typename CommandProgressUpdate3D::Pointer observer= CommandProgressUpdate3D::New();
    fractal->AddObserver( itk::ProgressEvent(), observer );
    }
